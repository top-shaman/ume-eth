...
class LikeButtonMain extends React.Component {
  constructor(props) {
    super(props)
    ...
    await this.state.interface.methods.likeMeme(this.state.userAccount, this.state.memeId)
      ...
      .then(() => {
        if(!this.state.userHasLiked) {
          ...
          this.liked.style.filter = 'sepia(1) brightness(0.4) saturate(10000%) hue-rotate(285deg)'
