...
contract ME is ERC1155 {
  ...
  function mintRespond(address accountFrom, address accountTo, string memory _postHash, string memory _originHash) public { // 2:1 Responder:Responded
    ...
    _mint(accountTo, RESPOND, 1, bytes(_postHash));
    // set event for RESPOND token TO
    emit MintRespond(accountTo, block.timestamp, _postHash, _originHash);
  }

  function mintCurate(address accountFrom, address accountTo, string memory _postHash, string memory _originHash) public { // 4:1 Responder:Responded
    require(accountFrom==msg.sender, 'Error: wrong account');
    require(minter!=msg.sender, 'Error: "we" cannot mint');
    require(accountFrom!=accountTo, 'Error: same account'); // doesn't mint tokens if tagged oneself
    require(bytes(_postHash).length > 0, 'Error: no post exists');
    // mint a CURATE token for curatee
    _mint(accountFrom, CURATE, 4, bytes(_postHash));
    ...
    _mint(accountTo, CURATE, 1, bytes(_postHash));
